...
- [x] File `backend/app/routers/hrs_data.py` created
- [x] Router prefix: `/hrs-data`, tags: `["hrs-data"]`
- [x] Endpoint 1: `GET /salary` - View own salary (authenticated user)
- [x] Endpoint 2: `GET /salary/history` - View salary history with trend (authenticated user)
- [x] Endpoint 3: `GET /salary/{employee_id}` - Admin view any employee's salary
- [x] Authorization implemented:
  - Endpoints 1-2: require authentication (`get_current_user`)
  - Endpoint 3: require admin role (`require_role("admin")`)
- [x] Input validation with Query parameters (year, month, from_month, to_month)
- [x] Default to current year/month if not provided (Endpoint 1)
- [x] Response models match Pydantic schemas (SalaryResponse, SalaryHistoryResponse)
- [x] Error handling: 400, 404, 422, 503 with descriptive messages
