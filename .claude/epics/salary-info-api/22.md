...
- [ ] File `backend/app/services/hrs_data_service.py` created
- [ ] `get_employee_salary(db, emp_id, year, month)` method implemented
- [ ] `get_salary_history(db, emp_id, year, from_month, to_month)` method implemented
- [ ] `calculate_trend(monthly_data)` helper function implemented
- [ ] Employee name lookup from database (fallback to "Unknown")
- [ ] Parallel API calls using `asyncio.gather()` for multi-month queries
- [ ] Trend analysis: average income/deductions/net, highest/lowest months
- [ ] Significant changes detection: >10% or >500K VND month-over-month
- [ ] Error handling: HRS API failures, employee not found, invalid period
- [ ] Logging: API calls, errors, trend insights
- [ ] Returns Pydantic schema-compatible dictionaries
...
- [ ] All 3 methods implemented in `backend/app/services/hrs_data_service.py`
- [ ] Employee name lookup from database works
- [ ] Parallel API calls implemented with `asyncio.gather()`
- [ ] Trend calculation correct (averages, highest/lowest, changes)
- [ ] Error handling for all failure cases
- [ ] Logging for API calls and errors
- [ ] Tested with single month query (emp_id=6204)
- [ ] Tested with multi-month history (12 months)
- [ ] Verified parallel performance (12 months < 5 seconds)
- [ ] Code follows existing service patterns
- [ ] Ready for use in API endpoints (Task 004)
